    ...
    <div className="space-y-6">
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-4">
            ...
            >
              {isConnected ? <CheckCircle2 className="h-6 w-6" /> : <AlertTriangle className="h-6 w-6" />}
            </div>
            <div>
          ...
          <a
            ...
            className={`inline-flex items-center justify-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition ${
              mercadoPagoConnectUrl
          ...
          >
            ...
            {isConnected ? "Reconectar Conta Mercado Pago" : "Conectar Mercado Pago (OAuth)"}
          </a>
        </div>
      </section>

      {/* 2. Conexão OAuth Explicada */}
      <section className="rounded-2xl border border-blue-100 bg-gradient-to-br from-blue-50/70 to-sky-50/50 p-6">
        <div className="flex items-start gap-3.5">
          <div className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-blue-600 text-white">
            <ShieldCheck className="h-4 w-4" />
          </div>
          <div>
            <h3 className="text-sm font-semibold text-blue-950">Conexão Recomendada: OAuth 2.0</h3>
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
        <div className="mb-5 flex items-center gap-3 border-b border-slate-100 pb-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Credenciais Manuais de API (Opcional)</h2>
            <p className="text-xs text-slate-500">Utilize caso prefira inserir seu Access Token e Public Key de produção manualmente</p>
          </div>
        </div>
            ...
            <label className="block text-sm font-medium text-slate-700">
              <span className="flex items-center justify-between">
                ...
                {integrationSettings?.hasMercadoPagoPublicKey && (
                  <span className="text-xs font-semibold text-emerald-600">✓ Chave salva</span>
                )}
              </span>
              <input
                type="text"
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
        <div className="mb-5 flex items-center gap-3 border-b border-slate-100 pb-4">
          ...
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Webhooks de Pagamento</h2>
            <p className="text-xs text-slate-500">Notificações em tempo real sobre status de PIX e pagamentos aprovados</p>
          </div>
        </div>
